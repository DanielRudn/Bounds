...
public class AndroidLauncher extends AndroidApplication implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener,
	...
	private byte[] newestMessage = null;
	private boolean hasNewInvite = false, justJoined = false, shouldShowWaitingRoom = false;
	...
	public void onRealTimeMessageReceived(RealTimeMessage arg0) {
		byte[] data = arg0.getMessageData();
		if(data[0] == 5)
		{
			// opponent sent 5
			Log.d("BOUNDS", "MESSAGE RECIEVED: 5");
		}
		else if(data[0] == 7)
		{
			// opponent sent 7
			Log.d("BOUNDS", "MESSAGE RECIEVED: 7");
		}
		newestMessage = data;
	}
